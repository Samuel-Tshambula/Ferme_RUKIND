...
export const EditProductForm: React.FC<EditProductFormProps> = ({ product, onClose, onProductUpdated }) => {
  const [formData, setFormData] = useState({
    name: product.name,
    description: product.description,
    category: product.category,
    inStock: product.stock > 0 ? 'true' : 'false',
    deliveryPrice: (product.deliveryPrice || 5).toString(),
    image: product.image,
    variants: product.variants && product.variants.length > 0 
      ? product.variants.map(v => ({
          unit: v.unit,
          price: v.price.toString(),
          minOrderQuantity: v.minOrderQuantity.toString(),
          description: v.description || ''
        }))
      : [{ unit: product.unit || 'kg', price: product.price.toString(), minOrderQuantity: (product.minOrderQuantity || 1).toString(), description: '' }]
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await productsAPI.update((product as any)._id || product.id.toString(), productData);
      onProductUpdated();
      onClose();
    } catch (error) {
      console.error('Erreur modification produit:', error);
      alert('Erreur lors de la modification du produit. Vérifiez la console pour plus de détails.');
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-6 border-b border-gray-700">
          <h3 className="text-xl font-bold text-white">Modifier le produit</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <div>
                <label htmlFor="edit-name" className="block text-sm font-medium text-gray-300 mb-2">Nom</label>
                <input
                  type="text"
                  id="edit-name"
                  name="name"
              ...
              <div>
                <label htmlFor="edit-category" className="block text-sm font-medium text-gray-300 mb-2">Catégorie</label>
                <select
                  id="edit-category"
                  name="category"
                  autoComplete="off"
              ...
              <div>
                <label htmlFor="edit-inStock" className="block text-sm font-medium text-gray-300 mb-2">Stock disponible</label>
                <select
                  id="edit-inStock"
                  name="inStock"
                  autoComplete="off"
              ...
              <div>
                <label htmlFor="edit-deliveryPrice" className="block text-sm font-medium text-gray-300 mb-2">
                  Prix livraison ($)
                </label>
                <input
                  type="number"
